...
PulseApp.ui.settings = (() => {
    ...
    function renderTabContent() {
        ...
        switch (activeTab) {
            ...
            case 'diagnostics':
                ...
                break;
            case 'backup-sources':
                content = renderBackupSourcesTab();
                break;
        }
        ...
        } else if (activeTab === 'backup-sources') {
            loadBackupSources();
        }
    }
    ...
    function renderBackupSourcesTab() {
        return `
            <div class="space-y-6">
                <!-- Header -->
                <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">Backup Storage Sources</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Overview of all backup storage locations being monitored by Pulse
                    </p>
                </div>
                
                <!-- Loading state -->
                <div id="backup-sources-loading" class="text-center py-8">
                ...
                <div id="backup-sources-content" class="hidden space-y-6">
                    <!-- PBS Sources -->
                    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                        <h4 class="text-md font-semibold text-gray-800 dark:text-gray-200 mb-3">
                            ...
                            Proxmox Backup Server (PBS) Sources
                        </h4>
                        <div id="pbs-sources-list" class="space-y-3">
                            <!-- PBS sources will be populated here -->
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                        <h4 class="text-md font-semibold text-gray-800 dark:text-gray-200 mb-3">
                            ...
                            Proxmox VE Storage Sources
                        </h4>
                        <div id="pve-sources-list" class="space-y-3">
                            <!-- PVE sources will be populated here -->
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                        <div class="flex items-start gap-2">
                            ...
                            <div>
                                <h5 class="text-sm font-semibold text-blue-800 dark:text-blue-200">Storage Type Information</h5>
                                <ul class="text-sm text-blue-700 dark:text-blue-300 mt-2 space-y-1">
                                    <li><strong>PBS:</strong> Dedicated backup server with deduplication and verification</li>
                                    <li><strong>NFS:</strong> Network File System storage</li>
                                    <li><strong>Local:</strong> Storage on Proxmox VE host</li>
                                    <li><strong>CIFS:</strong> Windows/SMB network shares</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Error state -->
                <div id="backup-sources-error" class="hidden">
                    <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                        <div class="flex items-start gap-2">
                            ...
                            <div>
                                <h5 class="text-sm font-semibold text-red-800 dark:text-red-200">Error Loading Backup Sources</h5>
                                <p class="text-sm text-red-700 dark:text-red-300 mt-1">
                                    Unable to load backup source information. Please check your configuration and try again.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    async function loadBackupSources() {
        const loadingEl = document.getElementById('backup-sources-loading');
        const contentEl = document.getElementById('backup-sources-content');
        const errorEl = document.getElementById('backup-sources-error');
        ...
        try {
            ...
            const state = PulseApp.state.get();
            if (!state) {
                throw new Error('No state data available');
            }
            
            // Process PBS sources
            const pbsSourcesList = document.getElementById('pbs-sources-list');
            if (pbsSourcesList && state.pbsDataArray) {
                pbsSourcesList.innerHTML = '';
                
                state.pbsDataArray.forEach((pbs, index) => {
                    const datastoreInfo = (pbs.datastores || []).map(ds => {
                        const snapCount = ds.snapshots ? ds.snapshots.length : 0;
                        return `${ds.name} (${snapCount} backups)`;
                    }).join(', ');
                    
                    const pbsItem = document.createElement('div');
                    pbsItem.className = 'p-3 bg-gray-50 dark:bg-gray-900 rounded border border-gray-200 dark:border-gray-700';
                    pbsItem.innerHTML = `
                        <div class="flex items-start justify-between">
                            <div>
                                <h5 class="font-medium text-gray-800 dark:text-gray-200">${pbs.pbsInstanceName || `PBS Instance ${index + 1}`}</h5>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                    Datastores: ${datastoreInfo || 'No datastores found'}
                                </p>
                                ${pbs.nodeName ? `<p class="text-xs text-gray-500 dark:text-gray-500 mt-1">Node: ${pbs.nodeName}</p>` : ''}
                            </div>
                            <span class="px-2 py-1 text-xs font-medium bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 rounded">
                                PBS
                            </span>
                        </div>
                    `;
                    pbsSourcesList.appendChild(pbsItem);
                });
                
                if (state.pbsDataArray.length === 0) {
                    pbsSourcesList.innerHTML = '<p class="text-sm text-gray-500 dark:text-gray-400">No PBS instances configured</p>';
                }
            }
            
            // Process PVE storage sources
            const pveSourcesList = document.getElementById('pve-sources-list');
            if (pveSourcesList && state.nodes) {
                pveSourcesList.innerHTML = '';
                
                const storageByType = {};
                
                // Collect all storage from all nodes
                state.nodes.forEach(node => {
                    if (node.storage && Array.isArray(node.storage)) {
                        node.storage.forEach(storage => {
                            if (storage.content && storage.content.includes('backup')) {
                                const type = storage.type || 'unknown';
                                if (!storageByType[type]) {
                                    storageByType[type] = [];
                                }
                                storageByType[type].push({
                ...
                // Display storage grouped by type
                Object.entries(storageByType).forEach(([type, storages]) => {
                    // Skip PBS type as it's shown in PBS section
                    if (type === 'pbs') return;
                    
                    const typeItem = document.createElement('div');
                    typeItem.className = 'p-3 bg-gray-50 dark:bg-gray-900 rounded border border-gray-200 dark:border-gray-700';
                    ...
                    const storageList = storages.map(s => {
                        ...
                        return `${status} ${s.name} (${s.node})`;
                    }).join(', ');
                    
                    typeItem.innerHTML = `
                        <div class="flex items-start justify-between">
                            <div>
                                <h5 class="font-medium text-gray-800 dark:text-gray-200">${typeLabel} Storage</h5>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                    ${storages.length} storage(s): ${storageList}
                                </p>
                            </div>
                            <span class="px-2 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-900/50 text-gray-700 dark:text-gray-300 rounded">
                                ${typeLabel}
                            </span>
                        </div>
                    `;
                    pveSourcesList.appendChild(typeItem);
                });
                
                if (Object.keys(storageByType).length === 0) {
                    pveSourcesList.innerHTML = '<p class="text-sm text-gray-500 dark:text-gray-400">No backup-enabled storage found</p>';
                }
            }
